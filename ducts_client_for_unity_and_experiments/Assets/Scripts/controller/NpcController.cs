...
public class NpcController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        DuctsRoot.ducts.OnOpenSetEventHandler("AckAllPlayersTransform"AllPlayersCommands[this.id]., (rid, eid, data) => {
            //Debug.Log("<AckAllPlayersTransform>AllPlayersCommands[this.id]. rid: " + rid + ", eid: " + eid + ", data.num: " + data.num + ", data.ver: " + data.str);
            if(id == data.num) {
        ...
        //DuctsRoot.ducts.AckAllPlayersTransform(AllPlayersCommands[this.id].d);
    }
    ...
    void Update()
    {
        ...
        {
            /*if ( Input.GetAxis("Horizontal") > 0.0f && Condition(false, "MAXRIGHT") ) 
            {
                //moveDirection.z = Input.GetAxis("Horizontal") * speedXZ;
                moveDirection.x = Input.GetAxis("Horizontal") * speedX;
            }
            else if ( Input.GetAxis("Horizontal") < 0.0f && Condition(false, "MINRIGHT") )
            {
            ...
            if ( Input.GetKey("w") && Condition(false, "MAXHEIGHT") ) 
            {
                //moveDirection.z = Input.GetAxis("Horizontal") * speedXZ;
                //Debug.Log("w");
            ...
            else if ( Input.GetKey("s") && Condition(false, "MINBOTTOM") )
            {
                //moveDirection.z = -1 * Input.GetAxis("Horizontal") * speedXZ;
                moveDirection.z = -1 * speedX;
